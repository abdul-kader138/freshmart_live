...
class Products extends MX_Controller
{
    ...
    */
    ...
    function view_adjustment()
    {
        ...
        $meta['page_title'] = "Adjustment Quantity";
        $data['page_title'] = "Adjustment Quantity";

        $this->load->view('commons/header', $meta);
        $this->load->view('view_adjustment', $data);
        $this->load->view('commons/footer');
    }

    function view_countquantity()
    {
        $data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
        $data['success_message'] = $this->session->flashdata('success_message');
        $data['warehouses'] = $this->products_model->getAllWarehouses();
        ...
        $this->load->view('commons/header', $meta);
        $this->load->view('view_countquantity', $data);
        $this->load->view('commons/footer');
    }
    ...
    function getdatatableajaxcost()
    {
        ...
        $this->datatables
            ->select("products.id as productid, products.image as image, products.code as code, products.name as name, categories.name as cname, products.cost, products.price, COALESCE(quantity, 0) as quantity, promotion.promo_title, alert_quantity, products.cf4 as barcode", FALSE)
            ->from('products')
            ->join('categories', 'products.category_id=categories.id', 'left')
            ->join('promotion', 'products.discount_id=promotion.promo_id', 'left')
    ...
    function getdatatableajaxadjustment()
    {
        ...
        $this->datatables->unset_column('productid');
        $this->datatables->add_column("Actions",
            "<center><a href='#' onClick=\"MyWindow=window.open('index.php?module=products&view=product_details&id=$1', 'MyWindow','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=600,height=600'); return false;\" class='tip' title='" . $this->lang->line("product_details") . "'><i class='icon-fullscreen'></i></a> <a href='index.php?module=products&view=save_adjust_inventory&id=$1' class='tip' title='Adjustment Quantity ADD'><i class='icon-list'></i></a></center>", "productid, image, code, name");
...
//            "<center><a id='$4 - $3' href='index.php?module=products&view=gen_barcode&code=$3&height=200' title='" . $this->lang->line("view_barcode") . "' class='barcode tip'><i class='icon-barcode'></i></a> <a href='#' onClick=\"MyWindow=window.open('index.php?module=products&view=product_details&id=$1', 'MyWindow','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=600,height=600'); return false;\" class='tip' title='" . $this->lang->line("product_details") . "'><i class='icon-fullscreen'></i></a> <a class='image tip' id='$4 - $3' href='" . $this->config->base_url() . "assets/uploads/$2' title='" . $this->lang->line("view_image") . "'><i class='icon-picture'></i></a> <a href='index.php?module=products&view=add_damage&product_id=$1' class='tip' title='" . $this->lang->line("add_damage_qty") . "'><i class='icon-filter'></i></a> <a href='index.php?module=products&view=edit&id=$1' class='tip' title='" . $this->lang->line("edit_product") . "'><i class='icon-edit'></i></a> <a href='index.php?module=products&view=delete&id=$1' onClick=\"return confirm('" . $this->lang->line('alert_x_product') . "')\" class='tip' title='" . $this->lang->line("delete_product") . "'><i class='icon-trash'></i></a></center>", "productid, image, code, name");
    ...
    function search_results($sr = "")
    {
        ...
        if ($this->input->post('pr_name')) {
            $sr_name = $this->input->post('pr_name');
        } else {
            $sr_name = $sr;
        }
        if ($this->input->get('search_term')) {
            $sr_name = $this->input->get('search_term');
        }
    ...
    {
        if ($this->input->get('srq')) {
            $srq = $this->input->get('srq');
        }
    ...
    function save_adjust_inventory($id = null, $warehouse_id = null)
    {
        ...
        $this->form_validation->set_rules('name', $this->lang->line("product_name"), 'required|xss_clean');
        $this->form_validation->set_rules('unit', $this->lang->line("product_unit"), 'required|xss_clean');
        $this->form_validation->set_rules('aj_quantity', "Adjustment Quantity", 'required|xss_clean');
        ...
        if ($this->form_validation->run() == true && $this->products_model->updateAdjustmentProduct($id, $data)) {
            $this->session->set_flashdata('success_message', "Quantity Adjustment successfully done.");
            ...
            $this->load->view('commons/header', $meta);
            $this->load->view('adjust_inventory', $data);
            $this->load->view('commons/footer');
    ...
    function upload_csv()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES["userfile"])) /*if($_FILES['userfile']['size'] > 0)*/ {
                ...
                $keys = array('code', 'name', 'category_code', 'unit', 'size', 'cost', 'price', 'alert_quantity', 'tax_rate', 'cf2', 'cf1');
            ...
            $ikeys = array('code', 'name', 'category_id', 'unit', 'size', 'cost', 'price', 'alert_quantity', 'tax_rate', 'cf2', 'cf1');

            $items = array();
            foreach (array_map(null, $pr_code, $pr_name, $pr_cat, $pr_unit, $pr_size, $pr_cost, $pr_price, $pr_aq, $pr_tax, $pr_cf2, $pr_cf1) as $ikey => $value) {
    ...
    function add_damage()
    {
        ...
        if ($this->input->get('warehouse_id')) {
            $data['warehouse_id'] = $this->input->get('warehouse_id');
        } else {
    ...
    {
        $category_id = $this->input->get('subcategory_id', TRUE);

        if ($rows = $this->products_model->getsubSubCategoriesByCategoryID($category_id)) {
            $ct[""] = '';
            foreach ($rows as $category) {
                $ct[$category->id] = $category->name;
            }
            $data = form_dropdown('subsubcategory', $ct, '', 'class="span4" id="subsubcategory" data-placeholder="' . $this->lang->line("select") . " " . $this->lang->line("subcategory") . '"');
        } else {
            $data = "";
        }
        echo $data;
    ...
    function getRacks()
    {
        $shelf_id = $this->input->get('shelf_id', TRUE);

        if ($rows = $this->products_model->getRackByShelfID($shelf_id)) {
            $ct[""] = '';

            foreach ($rows as $category) {
                $ct[$category->code] = $category->name;
            }
            $data = form_dropdown('cf3', $ct, '', 'class="span4" id="cf3" data-placeholder="' . $this->lang->line("select") . "' Rack");
        } else {
            $data = "";
        }
        echo $data;
