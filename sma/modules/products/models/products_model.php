...
class Products_model extends CI_Model
{
    ...
    public function getWarehousesProductByID($id, $warehouseId)
    {

        $q = $this->db->get_where('warehouses_products', array('product_id' => $id, 'warehouse_id' => $warehouseId), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
    ...
    public function updateAdjustmentProduct($id, $data = array())
    {

        $new_data = $this->getProductByID($id);
        if ($data['sign'] == '+') {
            $qty = (string)($new_data->quantity +  $data['adjust_qnt']);
        } else {
            $qty =(string) ($new_data->quantity - $data['adjust_qnt']);
        }

        $date = date('Y-m-d H:i:s');
        $productData = array('adjust_qnt' => $data['adjust_qnt'], 'quantity' => $qty);

        //make warehouse adjust qty
        $whProductInfo = $this->getWarehousesProductByID($id, $data['warehouse_id']);

        if ($data['sign'] == '+') {
            $product_qnt = (string)($whProductInfo->quantity + $data['adjust_qnt']);
            $adjust_qty_add=(string)$data['adjust_qnt'];
        } else {

            $product_qnt = (string)($whProductInfo->quantity - $data['adjust_qnt']);
            $adjust_qty_remove=(string)$data['adjust_qnt'];
        }
        // Product data
        $WhProductData = array('quantity' => $product_qnt);

        if ($whProductInfo != false) {
            $adjustment_obj = array(
                'product_id' => $id,
    ...
    public function getsubSubCategoriesByCategoryID($category_id)
    {
        $q = $this->db->get_where("sub_subcategories", array('sub_category' => $category_id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
