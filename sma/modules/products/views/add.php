		...
		$('#subcategory').change(function() {
			...
			$('#loading').show();
					$.ajax({
					  type: "get",
					  async: false,
					  url: "index.php?module=products&view=getsubSubCategories",
					  data: { <?php echo $this->security->get_csrf_token_name(); ?>: "<?php echo $this->security->get_csrf_hash() ?>", category_id: v },
					  dataType: "html",
					  success: function(data) {
						  alert(data);
						if(data != "") {
							$('#subsubcat_data').empty();
							$('#subsubcat_data').html(data);
						} else {
							$('#subsubcat_data').empty();
							var default_data = '<select name="subsubcategory" class="span4" id="subsubcategory" data-placeholder="<?php echo $this->lang->line("select_category_to_load"); ?>"></select>';
							$('#subsubcat_data').html(default_data);
							//bootbox.alert('<?php echo $this->lang->line('no_subcategory'); ?>');
						}
					  },
					  error: function(){
...
<div class="control-group">
  <label class="control-label" for="subsubcategory">Sub Subcategory</label>
  <div class="controls" id="subsubcat_data"> <?php 
	   		$sct1[""] = '';
	   		
			echo form_dropdown('subsubcategory', $sct1, '', 'class="span4" id="subsubcategory"  data-placeholder="'.$this->lang->line("select_category_to_load").'"');  ?> </div>
</div>
...
<div class="control-group">
  ...
  <div class="controls">  <?php 
	  $sh[''] = "";
		...
		echo form_dropdown('cf1', $sh, (isset($_POST['cf1']) ? $_POST['cf1'] : ""), 'class="tip chzn-select span4" id="cf1" data-placeholder="'.$this->lang->line("select")." ".$this->lang->line("shelf").'" title="'.$this->lang->line("pr_shelf_tip").'" required="required" data-error="'.$this->lang->line("shelf").' '.$this->lang->line("is_required").'"'); ?> </div>
</div>
...
<div class="control-group">
  ...
  <div class="controls" id="rack_data"> <?php 
			...
			echo form_dropdown('rack', $sct, '', 'class="span4" id="cf3"  data-placeholder="Rack"');  ?> </div>
</div>
