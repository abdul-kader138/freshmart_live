...
//Add new pos sale
    ...
    function index()
    {
        if ($this->input->get('suspend_id')) {
            $data['sid'] = $this->input->get('suspend_id');
        } else {
            $data['sid'] = NULL;
        }
...
//				$customer_name = $customer_details->name;
            ...
            for ($i = 1; $i <= 500; $i++) {
                if ($this->input->post($quantity . $i) && $this->input->post($product . $i) && $this->input->post($unit_price . $i)) {
                    ...
                    if (DISCOUNT_METHOD == 1 && DISCOUNT_OPTION == 2) {
                        ...
                        $prm = $product_details->discount_amount;
                        $prmType = $product_details->promo_type;
                        $prmID[] = $product_details->promo_id;

                        $ds = $ds_details->discount + $prm;
                        ...
                        if ($prmType == 2) {
                            ...
                            if ($product_details->promo_id != '' || $product_details->promo_id) {
                                $promo[] = $product_details->promo_title;
                                $getBuy[] = $this->input->post($quantity . $i);
                            } else {
                                $promo[] = "";
                                $getBuy[] = 0;
                            }
            ...
            $items = array();
            foreach (array_map(null, $product_id, $product_code, $product_name, $product_unit, $tax_rate_id, $tax, $inv_quantity, $inv_unit_price, $inv_gross_total, $val_tax, $serial, $val_ds, $discount, $dsID, $getBuy, $promo) as $key => $value) {
            ...
            if (DISCOUNT_METHOD == 1 && DISCOUNT_OPTION == 1) {
                ...
                $ds = $ds_dts->discount + $product_details->discount_amount;
                $dsTp = $ds_dts->type;
            ...
            $saleDetails = array('reference_no' => $reference_no,
                ...
                'return_amount' => $this->input->post('items_return_val'),
                'return_ref' => $this->input->post('items_return_ref')
            );
...
//                if ($saleID = $this->pos_model->addSale($saleDetails, $items, $warehouse_id, $did,$credit_obj)) {
    ...
    function scan_product()
    {
        ...
        if ($prodd = $this->pos_model->getProductByCode($code)) {
            ...
            if ($customer->cf1 == 'All Products') {
                $prodd->discount_amount = $customer->cf2;
                $discount_type = $customer->cf5;
            } else if ($customer->cf1 == 'Paragon Products' && $prodd->cf6 == '1') {

                $prodd->discount_amount = $customer->cf2;
                $discount_type = $customer->cf5;
            } else {
                $prodd->discount_amount = $prodd->discount_amount ? $prodd->discount_amount : 0.00;
            ...
            $discount = $prodd->discount_amount;
            $promo_type = $prodd->promo_type;
            $promo_get = $prodd->promo_get;
            $promo_buy = $prodd->promo_buy;
            if ($product_id < 10) {
                $product_id = "0" . (($product_id * 100) / 100);
            }
            if ($category_id < 10) {
            ...
            $product = array('product_name' => $product_name,
                ...
                'promo_get' => $promo_get,
                'promo_buy' => $promo_buy,
                'discount_new_type' => $discount_type
            );
    ...
    function getProductBYname()
    {
        ...
        if ($prodd = $this->pos_model->getProductByNameForSearch($name)) {

            echo json_encode($prodd);
    ...
    function getOrderdetails()
    {
        ...
        $order = $this->pos_model->getInvoiceByRefID($id);

        if ($prodd = $this->pos_model->getAllInvoiceItemsForReturn($order->id)) {
            ...
            <table border="0" cellpadding="0" cellspacing="0" class="protable" id="returnAmount">
                ...
                <?php
                foreach ($prodd as $value) {
                    ?>
                    <tr>
                        <td><input type="hidden" name="return_price[]" value="<?php echo $value->unit_price; ?>"/><input
                                type="hidden" name="return_product_id[]"
                                value="<?php echo $value->product_id; ?>"/><input type="hidden"
                                                                                  id="order_items<?php echo $value->product_id; ?>"
                                                                                  name="orderitems_id[]"
                                                                                  value="<?php echo $value->id; ?>"/><?php echo $value->product_name; ?>
                        </td>
                        <td><?php echo $value->unit_price; ?></td>
                        <td><?php echo $value->quantity; ?></td>
                        <td><input type="text" id="ret_<?php echo $value->product_id; ?>" name="returnproduct[]"
                                   style="width:50px;"/></td>
    ...
    function price($code = NULL)
    {
        ...
        if ($customer->cf1 == 'All Products') {
            $products->discount_amount = $customer->cf2;
            $discount_type = $customer->cf5;
        } else if ($customer->cf1 == 'Paragon Products' && $products->cf6 == '1') {

            $products->discount_amount = $customer->cf2;
            $discount_type = $customer->cf5;
        } else {

            $products->discount_amount = $products->discount_amount ? $products->discount_amount : 0.00;
            $discount_type = DEFAULT_DISCOUNT;
        }
        ...
        $discount = $products->discount_amount;
        $promo_type = $products->promo_type;
        $promo_get = $products->promo_get;
        $promo_buy = $products->promo_buy;

        $data = array('price' => $price, 'name' => $name, 'code' => $code, 'tax_rate' => $tax_rate, 'discount' => $discount, 'promo_type' => $promo_type, 'promo_get' => $promo_get, 'promo_buy' => $promo_buy, 'discount_new_type' => $discount_type);
...
//view inventory as html page
    ...
    function today_sale()
    {
        ...
        if ($cc_sale = $this->pos_model->getTodayCCSales()) {
            $data['ccsales'] = $cc_sale->total;
        } else {
            $data['ccsales'] = "0.00";
        }
        if ($cash_sale = $this->pos_model->getTodayCashSales()) {
            $data['cashsales'] = $cash_sale->total;
        } else {
            $data['cashsales'] = "0.00";
        }
        if ($ch_sale = $this->pos_model->getTodayChSales()) {
            $data['chsales'] = $ch_sale->total;
        } else {
            $data['chsales'] = "0.00";
        }
    ...
    function add_customer()
    {
        ...
        if (empty($cusData[8])) {
            $error = $this->lang->line("country") . " " . $this->lang->line("is_required");
        }
        if (empty($cusData[7]) || !is_numeric($cusData[7])) {
            $error = $this->lang->line("postal_code") . " " . $this->lang->line("is_required");
        }
        if (empty($cusData[6])) {
            $error = $this->lang->line("state") . " " . $this->lang->line("is_required");
        }
        if (empty($cusData[5])) {
            $error = $this->lang->line("city") . " " . $this->lang->line("is_required");
        }
        if (empty($cusData[4])) {
            $error = $this->lang->line("address") . " " . $this->lang->line("is_required");
        }
        if (empty($cusData[2]) || !is_numeric($cusData[2])) {
            $error = $this->lang->line("phone") . " " . $this->lang->line("is_required");
        }
        if (empty($cusData[1])) {
            $error = $this->lang->line("name") . " " . $this->lang->line("is_required");
        }
        if (empty($cusData[0])) {
            $error = $this->lang->line("company") . " " . $this->lang->line("is_required");
        }
    ...
    function save_return()
    {
        ...
        $sales_item_id = $this->input->get('orderitems_id');
        $product_id = $this->input->get('return_product_id');
        $price = $this->input->get('return_price');
        $total = $this->pos_model->return_items($sales_item_id, $qnt, $product_id, $price);
        echo $total;
