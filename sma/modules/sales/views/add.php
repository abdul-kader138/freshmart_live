...
$('#customer_s').change(function(e) {
			...
			$.ajax({
                    type: "get",
                    async: false,
                    url: "<?php echo $this->config->base_url(); ?>index.php?module=sales&view=get_due",
                    data: {<?php echo $this->security->get_csrf_token_name(); ?>: "<?php echo $this->security->get_csrf_hash() ?>", cus_id: cus_id},
                    dataType: "json",
                    success: function(data) {

                        due = parseFloat(data.due);
                        $('#due').val(due)
					    var payable = parseFloat($('#gtotal').val());
						var paytotal = payable + due;
						$('#gtotal').val(paytotal.toFixed(2)); 
...
<?php } ?>
                   due = parseFloat($('#due').val());
                gtotal = ((total + total_tax1 + total_tax2) - total_discount) + shipping + due;
...
<?php } ?>
						...
						$("#dyTable").on("focus", 'input[id^="price-"]', function() {
							var rID = $(this).attr('id');
                            var r_id = rID.split("-");
                            var rw_no = r_id[1];
							
                            oprice = $('#price-' + rw_no).val();
							
                        });
                        $("#dyTable").on("blur", 'input[id^="price-"]', function() {
                            var rID = $(this).attr('id');
                            var r_id = rID.split("-");
                            var rw_no = r_id[1];
							
                            var nqty = $('#quantity-' + rw_no).val();
                            var rprice = $('#price-' + rw_no).val();
                             
                            var oldrowtotal = nqty * oprice;
                            var newrowtotal = nqty * rprice;
