...
function quote_to_invoice()
   ...
   function discounts()
   {
    ...
    function get_due()
   {
	   if($this->input->get('cus_id')) { $id = $this->input->get('cus_id'); }
	  
	   $new_discount_details = $this->sales_model->getDueByCustomerID($id);
	   $due = $new_discount_details->total - $new_discount_details->paid;
	   
	    echo json_encode(array('due'=>$due));
