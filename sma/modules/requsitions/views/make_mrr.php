<?php
$reference_no = array(
    'name' => 'mr_reference_no',
    'id' => 'mr_reference_no',
    'value' => 'MR-'.sprintf("%06d", $inv->id),
    'class' => 'span4',
    'required' => "required",
	'readonly' => "readonly",
...
<h3 class="title">Material Receiving</h3>
<p>Material Receiving</p>

<?php $attrib = array('class' => 'form-horizontal', 'id' => 'addSale_form');
echo form_open("module=inventories&view=make_mrr&id=" . $id, $attrib);
?>
...
<div class="control-group">
    <label class="control-label" for="mr_reference_no"><?php echo $this->lang->line("reference_no"); ?></label>
    <div class="controls"> <?php echo form_input($reference_no); ?> </div>
</div>
<input type="hidden" name="reference_no" value="<?php echo $inv->reference_no; ?>" />
<div class="control-group" style="display:none">
    ...
    <div class="controls">  <?php
        ...
        echo form_dropdown('warehouse', $wh, (isset($_POST['warehouse']) ? $_POST['warehouse'] : $inv->warehouse_id), 'id="warehouse_s" data-placeholder="' . $this->lang->line("select") . ' ' . $this->lang->line("warehouse") . '" required="required" readonly="readonly" data-error="' . $this->lang->line("warehouse") . ' ' . $this->lang->line("is_required") . '"');
        ?> </div>
</div>

<div class="control-group">
    <label class="control-label" for="supplier_name"><?php echo $this->lang->line("supplier"); ?></label>
    <div class="controls"> <?php echo form_input($supplier_name); ?> </div>
</div>
...
?>
            ...
            <th style="width: 20px;"><input type="checkbox" name="all" id="checkall" value="all" /></i></th>
            </thead>
            <tbody id="checkboxes">
                <?php
                $r = 1;
                foreach ($inv_products as $prod) {
                    ...
                    if($prod->mr_item_status){ $chec = 'checked'; } else{ $chec = 'checked';}
                    ...
                    echo '<td><input class="input-block-level text-center" name="quantity' . $r . '" type="text" value="' . $prod->quantity . '"></td>
...
<div class="control-group">
    <div class="controls">
       ...
       if($this->ion_auth->in_group(array('admin', 'owner', 'approver'))){
         if($inv->mr_status == 1){ ?>
                    <a href="index.php?module=inventories&view=mrr_verify_approve&id=<?php echo $inv->id ?>"><button type="button" class="btn btn-primary" style="padding: 6px 15px;">Approve</button></a>
					&nbsp;
<a href="index.php?module=inventories&view=cancel_mrr&id=<?php echo $inv->id ?>"><button type="button" class="btn btn-primary" style="padding: 6px 15px;">Cancel</button></a>
