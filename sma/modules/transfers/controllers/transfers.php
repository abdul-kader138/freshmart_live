...
//                "<center><a href='#' onClick=\"MyWindow=window.open('index.php?module=transfers&view=view_transfer&id=$1','MyWindow','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=1000,height=600'); return false;\" title='" . $this->lang->line("view_transfer") . "' class='tip'><i class='icon-fullscreen'></i></a> <a href='index.php?module=transfers&view=transfer_pdf&id=$1' title='" . $this->lang->line("download_pdf") . "' class='tip'><i class='icon-file'></i></a> <a href='index.php?module=transfers&view=edit&id=$1' title='" . $this->lang->line("edit_transfer") . "' class='tip'><i class='icon-edit'></i></a> <a href='index.php?module=transfers&view=delete_transfer&id=$1' onClick=\"return confirm('" . $this->lang->line('alert_x_transfer') . "');\" title='" . $this->lang->line("delete_transfer") . "' class='tip'><i class='icon-trash'></i></a></center>", "id")
                "<center><a href='#' onClick=\"MyWindow=window.open('index.php?module=transfers&view=view_transfer&id=$1','MyWindow','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=1000,height=600'); return false;\" title='" . $this->lang->line("view_transfer") . "' class='tip'><i class='icon-fullscreen'></i></a> <a href='index.php?module=transfers&view=transfer_pdf&id=$1' title='" . $this->lang->line("download_pdf") . "' class='tip'><i class='icon-file'></i></a></center>", "id")
            ->unset_column('id')
            ->unset_column('fcode')
            ->unset_column('tcode');
        ...
        if ($this->form_validation->run()) {
            ...
            for ($i = 1; $i <= TOTAL_ROWS; $i++) {
                ...
                if ($this->input->post($quantity . $i) && $this->input->post($product . $i) && $this->input->post($unit_cost . $i)) {
                    ...
                    if ($this->transfers_model->getProductQuantity($product_details->id, $from_warehouse)) {
                        $get_product_qty = $this->transfers_model->getProductQuantity($product_details->id, $from_warehouse);
                        $checkQty = $get_product_qty['quantity'];
                        if ($checkQty == "" || $checkQty == null) $checkQty = 0.0;
                    }

//                    if (RESTRICT_SALE) {
//                        if ($checkQty < (float)$this->input->post($quantity . $i)  && $product_details->track_quantity == 1) {
                    if ($checkQty < $this->input->post($quantity . $i)) {
                        $check = TRUE;
                        $product_name = $product_details->name;
                    }
//                    }
//                    $get_product_qty = $this->getProductQuantity($product_details->id, $from_warehouse);
                    if (TAX1) {
        ...
        if ($this->form_validation->run() == true && !$check && $this->transfers_model->transferProducts($transferDetails, $items, $obj)) {
...
//                       }
        ...
        if ($this->form_validation->run() == true && !$check && $this->transfers_model->updateTransfer($id,$transferDetails, $items, $obj)) {
