...
class Categories_model extends CI_Model
{
	...
	{
		$q = $this->db->get("sub_subcategories");
		if($q->num_rows() > 0) {
			foreach (($q->result()) as $row) {
				$data[] = $row;
			}
	...
	public function getsubSubCategoriesBysubCategoryID($category_id) 
	{
	...
	public function getsubSubCategoryByID($id) 
	{
		...
		$q = $this->db->get_where("sub_subcategories", array('id' => $id), 1); 
		  if( $q->num_rows() > 0 )
		  {
			return $q->row();
		  } 
	...
	public function addsubSubCategory($category, $name, $code)
	{

		if($this->db->insert("sub_subcategories", array('sub_category' => $category,'code' => $code, 'name' => $name))) {
			return true;
		} else {
			return false;
		}
	...
	public function updatesubSubCategory($id, $data = array())
	{
		...
		$this->db->where('id', $id);
		if($this->db->update("sub_subcategories", $categoryData)) {
			return true;
		} else {
			return false;
		}
	...
	{
		if($this->db->delete("sub_subcategories", array('id' => $id))) {
			return true;
		}
	return FALSE;
	}
