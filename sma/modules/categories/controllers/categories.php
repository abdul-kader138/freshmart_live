...
*/
	...
	function subsubcategories()
   {
	  ...
	  $data['page_title'] = $this->lang->line("subcategories");
      $this->load->view('commons/header', $meta);
      $this->load->view('subsubcategories', $data);
      $this->load->view('commons/footer');
   }
   ...
   function getsubsubcategory()
   {
	   ...
	   $this->load->library('datatables');
	   $this->datatables
			->select("sub_subcategories.id as id, sub_subcategories.code as scode, sub_subcategories.name as sname, subcategories.name as cname")
			->from("sub_subcategories")
			->join('subcategories', 'sub_subcategories.sub_category = subcategories.id', 'left')
			->group_by('sub_subcategories.id');
			
		if($parent_id) { $this->datatables->where('category_id', $parent_id); }	
			
		$this->datatables->add_column("Actions", 
			"<center>			<a href='index.php?module=categories&amp;view=edit_subsubcategory&amp;id=$1' class='tip' title='".$this->lang->line("edit_subcategory")."'><i class=\"icon-edit\"></i></a>
							    <a href='index.php?module=categories&amp;view=delete_subsubcategory&amp;id=$1' onClick=\"return confirm('". $this->lang->line('alert_x_subcategory') ."')\" class='tip' title='".$this->lang->line("delete_subcategory")."'><i class=\"icon-remove\"></i></a></center>", "id")
			->unset_column('id');
   ...
   /*************** Sub Subcategory *************************/
	...
	function add_subsubcategory()
	{
		...
		if ( $this->form_validation->run() == true && $this->categories_model->addsubSubCategory($category, $name, $code))
		{ //check to see if we are creating the customer
			//redirect them back to the admin page
			$this->session->set_flashdata('success_message', 'Sub Subcategory Added successfully');
			redirect("module=categories&view=subsubcategories", 'refresh');
		}
		else
		{ //display the create customer form
			//set the flash data error message if there is one
		...
		$data['categories'] = $this->categories_model->getAllSubCategories();
		$meta['page_title'] = $this->lang->line("new_subcategory");
		$data['page_title'] = $this->lang->line("new_subcategory");
		$this->load->view('commons/header', $meta);
	...
	function edit_subsubcategory($id = NULL)
	{
		if($this->input->get('id')) { $id = $this->input->get('id'); }
		...
		if ( $this->form_validation->run() == true && $this->categories_model->updatesubSubCategory($id, $data))
		{ //check to see if we are updateing the customer
			//redirect them back to the admin page
			$this->session->set_flashdata('success_message', 'Sub Subcategory update successfully');
			redirect("module=categories&view=subsubcategories", 'refresh');
		}
		else
		{ //display the update form
		...
		$data['subcategory'] = $this->categories_model->getsubSubCategoryByID($id);
		$data['categories'] = $this->categories_model->getAllSubCategories();
		$meta['page_title'] = $this->lang->line("update_subcategory");
		$data['id'] = $id;
		$data['page_title'] = $this->lang->line("update_subcategory");
		$this->load->view('commons/header', $meta);
		$this->load->view('edit_subsubcategory', $data);
		$this->load->view('commons/footer');
		
		}
	}
	
	function delete_subsubcategory($id = NULL)
	{
		if (DEMO) {
			$this->session->set_flashdata('message', $this->lang->line("disabled_in_demo"));
			redirect('module=home', 'refresh');
		...
		if ( $this->categories_model->deletesubSubCategory($id) )
		{  
			$this->session->set_flashdata('success_message', 'Delete Sub Subcategory');
			redirect("module=categories&view=subsubcategories", 'refresh');
		}
